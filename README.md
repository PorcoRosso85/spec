...
### 3. Skeleton → Declare placement

Update `docs/structure/.gen/skeleton.json`:
...
Every PR is checked by 4 required jobs:
...
4. **traceability-gen** - Regenerates and validates traceability.json
...
2. **Always follow the 3-step process** - catalog → ADR → skeleton
3. **One slot = One responsibility** (SRP - Single Responsibility Principle)
4. **One slot = One owner** - Clear accountability
5. **No circular dependencies** - Validated by CI
