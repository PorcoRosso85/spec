...
let
  ...
  # Output: CUE string with required and actual both set to all keys
  genConsumerAPIVerifyCue = specKeys:
    let
      # Format as CUE string list
      keyItems = builtins.map (key: ''"${key}"'') specKeys;
      keyList = builtins.concatStringsSep ",\n\t\t" keyItems;
    in ''
      ...
      // Auto-generated by nix/lib/integration.nix
      // Source: self.spec keys (real data, clean)
      ...
      integrationInput: det.#ConsumerAPIInput & {
      	...
      	required: list.MinItems(2)
      	actual: list.MinItems(2)
      }
    '';
  
  # Helper: Generate CUE file for DoD2 negative (missing key)
  # Input: list of spec keys, key to remove (e.g., "spec.urn.envPath")
  # Output: CUE string with required having all keys, actual missing one
  genConsumerAPINegativeCue = specKeys: missingKey:
    let
      # Required: all keys
      requiredItems = builtins.map (key: ''"${key}"'') specKeys;
      requiredList = builtins.concatStringsSep ",\n\t\t" requiredItems;
      
      # Actual: all keys except missingKey
    ...
    in ''
      ...
      // Auto-generated by nix/lib/integration.nix
      // Source: self.spec keys with intentional missing key (malicious)
