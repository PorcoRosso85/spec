...
let
  ...
  # Excludes: sandboxes (Phase 8 special directory for CI要件データ only)
  listFeatDirs =
    let
      featRoot = "${self}/spec/urn/feat";
      entries = builtins.readDir featRoot;
      # Filter only directories and exclude sandboxes
      dirs = builtins.filter (name: entries.${name} == "directory" && name != "sandboxes") (
  ...
  extractFeatMeta =
    ...
    let
      ...
      slug =
        if slugMatch != null && builtins.length slugMatch > 0 then
          builtins.head slugMatch
        else
          throw "Failed to extract slug from ${featFile}";
    ...
    let
      ...
      flattenAttrs =
        ...
        builtins.concatMap (
          ...
          in
          if builtins.isAttrs value && !(isDerivation value) then
  ...
  extractManifest =
    let
      ...
      manifestJson = pkgs.runCommand "manifest.json" { buildInputs = [ cue ]; } ''
        cd ${self}
